...
abstract class Sorter {
    ...
    void initializePart2(int[] A) {
        this.original = A;
        this.n = A.length;  // Set n to the length of the original array
        // We clone, so that A may be safely passed to multiple sorters
        this.A = Arrays.copyOfRange(original, 0, n);
    }
    */
    ...
    // Comparison that count
    boolean compare(int a, int b, String operator) {
        comparisons++; 
        switch (operator) {
            case ">": 
                return a > b;   // Returnerer 'true' om a er større enn b, 'false' ellers (Hvis venstre verdi er større enn høyre verdi, kall på swap() etterpå (fordi de større verdiene skal ligge til høyre i arrayet))
            case "<":
                return a < b;   // Returnerer 'true' om a er mindre enn b, 'false' ellers
            case ">=":
                return a >= b; // Alternativ: "Hvis venstreverdien er større enn (eller lik) høyreverdien, returner 'true", osv. 
            case "<=":
                return a <= b; // osv...
            case "==":
                return a == b; 
